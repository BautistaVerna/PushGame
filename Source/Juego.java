...
public class Juego {
    ...
    private void jugarTurno(Jugador jugador, Scanner scanner) {
        while (true) {
            ...
            if (opcion == 2) {
                // Si el jugador decide terminar el turno
                terminarTurno(jugador);
                ...
                // Mostrar opciones para colocar la carta en una fila
                switch (mostrarOpciones(scanner)) {
                    case 1:
                        colocarCartaEnFila(jugador, carta, scanner);
                        break;
    ...
    // Terminar el turno del jugador y añadir filas activas al botín
    private void terminarTurno(Jugador jugador) {
        for (Fila fila : filasActivas) {
            jugador.getBotin().agregarCartas(fila.getCartas()); // Añadir las cartas de las filas al botín
        }
        filasActivas = inicializarFilas(); // Reiniciar filas para el siguiente turno

        // Mostrar botin
        mostrarBotin(jugador);
    }
    ...
    // Método para mostrar el botín del jugador con el conteo de cartas por color
    private void mostrarBotin(Jugador jugador) {
        System.out.println("Botín del jugador " + jugador.getNombre() + ":");

        Map<String, Integer> conteoPorColor = jugador.getBotin().contarCartasPorColor();

        for (Map.Entry<String, Integer> entry : conteoPorColor.entrySet()) {
            System.out.println("Color: " + entry.getKey() + " - Cantidad de cartas: " + entry.getValue());
        }
    }
